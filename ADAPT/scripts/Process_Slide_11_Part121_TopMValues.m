...
function out = Process_Slide_11_Part121_TopMValues(DashBoard,end_Time, Decision, Reason, class, Pending, fontsize,NDays)      
 ...
 x4 = 0.04;       
        ...
        inds_121 = find(class == 2 & Decision == 1 & Departure_from_Deploy' > (lastDay - 11));   % Part 121 Approvals in last 10 days (only show if any approvals in last 10 days)
        n_121 = length(inds_121);
                 CallSigns = [];
                for k = inds_121;
                ...
                nCall_TN = length(CallSign_List_TN);  % number of unique part 121 call signs
        ...
        figure(011); close(011); figure(011)
        fh = figure(011);
        plot(0,0,'ko'); hold on
        plot(0,0,'md')
        plot(0,0,'b*')
        ...
        CounterValue = 0;
        CallCollect = {};
        for m = 1:nCall_TN  % Loop through each call sign that had at least one part 121 approval in the last 10 days
            inds = find(strcmp(CallSign_List_TN{m},DashBoard.TailNumber) & Decision == 1 & class == 2);  % Total number of approved Part 121 requests from current Tail Number (
            if length(inds) == 0
                inds = find(strcmp(CallSign_List_TN{m},DashBoard.SICAO) & Decision == 1);  % If no Tail Number, then use ICAO
                         
            end
            inds_121 = find(class(inds) == 2 & (Departure_from_Deploy(inds)' > lastDay - NDays));  % Number of approved applications in last NDays (NDays is current M history window)
...
%                 plot(Departure_from_Deploy(inds(inds_135)),m*ones(1,length(inds_135)),'g+')         
            ...
            if n_Days_121 >= 3
                ...
                plot(Departure_from_Deploy(inds(inds_121)),CounterValue*ones(1,length(inds_121)),'ko');
    ...
    %             n_Days_135 = length(unique(floor(Departure_from_Deploy(inds(inds_135)))));
                ...
                text(xt1,CounterValue-.2,[num2str(length(inds)) ' /' num2str(n_Days_121)],'Fontsize',14);
    %             inds_135
    %             inds_91
            end
        ...
        %legend('Part 121','Part 129','Part 91','Part 135')
        %title('Approved Transaction History for Part 121 Aircraft')
        title(sprintf('Part 121 10-Day Transaction History with %1.0f-Day M Totals: %1s',NDays,DashBoard.Filename),'Interpreter','None','FontSize',fontsize);
